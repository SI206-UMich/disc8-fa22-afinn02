...
def getLink(soup):
    ...
    ulr = a_tag.get('href')
    return "https://en.wikipedia.org" + ulr
...
def getAdmissionsInfo2019(soup):
    d = {}
    box_info = soup.find("table", class_ = "toccolours")
    names = box_info.find_all('tr')
    for tr_tag in names[1:]:
        td_list = tr_tag.find_all('td')
        schoolName = td_list[0].text.strip()
        foundYear = td_list[1].text.strip()
        d[schoolName] = foundYear
    return d
...
def main():
    ...
    link_ = "https://en.wikipedia.org/wiki/University_of_Michigan"
